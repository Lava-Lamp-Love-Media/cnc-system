...
<style>
    ...
    body.director-page .content-header {
        display: none;
    }

    body.director-page .main-footer {
        display: none;
    }
    ...
    .dir-pal::-webkit-scrollbar {
        width: 4px;
    }

    .dir-pal::-webkit-scrollbar-thumb {
        background: #a0b0c4;
        border-radius: 2px;
    }
    ...
    .dc-hd {
        background: #b0bece;
        border-bottom: 2px solid #98aabf;
        padding: 4px 12px;
        display: flex;
        align-items: center;
        gap: 8px;
    ...
    .dstep.sel {
        outline: 3px solid #2060c8;
        outline-offset: 1px;
        z-index: 2;
    }

    .dstep.dov {
        outline: 2px dashed #3060c8;
    ...
    .dbbl::before {
        ...
        border-top-color: #a0b0c4;
    }

    /* Tracking box */
    .d-tracking {
        background: #fff;
        border: 2px solid #90a8c0;
        border-radius: 8px;
        padding: 12px 18px;
        font-size: 13px;
        font-weight: 700;
        ...
        <div class="dir-mbd">
            ...
            <div class="dir-mf"><label>Machine</label>
                <select>
                    ...
                    <option>HAAS VF-4 (5-Axis Mill)</option>
                    <option>Mazak ST-20 (Lathe)</option>
                    <option>DMG VMC-500</option>
                    <option>Brother TC-S2D</option>
                </select>
            </div>
...
════════════════════════════════════════ */
    ...
    function dirClickParent(qid) {
        ...
        dirActiveId = qid;
        var q = DIR_QUOTES.find(function(x) {
            return x.id === qid;
        });
        if (q) {
    ...
    function dirLoadJO(joid, qid) {
        ...
        var jo = q ? q.children.find(function(x) {
            return x.id === joid;
        }) : null;
        if (!jo) return;
        var items = [];
        for (var i = 0; i < jo.holes; i++) items.push({
            type: 'hole',
            label: 'Hole',
            icon: 'fa-circle-notch',
            cls: 'ss-hole'
        });
        for (var i = 0; i < jo.taps; i++) items.push({
            type: 'tap',
            label: 'Tap',
            icon: 'fa-screwdriver',
            cls: 'ss-tap'
        });
        for (var i = 0; i < jo.machines; i++) items.push({
            type: 'machine',
            label: 'Machine',
            icon: 'fa-cog',
            cls: 'ss-machine'
        });
        for (var i = 0; i < jo.ops; i++) items.push({
            type: 'op',
            label: 'Operation',
            icon: 'fa-tools',
            cls: 'ss-op'
        });
        for (var i = 0; i < jo.threads; i++) items.push({
            type: 'thread',
            label: 'Threading',
            icon: 'fa-ring',
            cls: 'ss-thread'
        });
        for (var i = 0; i < jo.items; i++) items.push({
            type: 'item',
            label: 'Item',
            icon: 'fa-box',
            cls: 'ss-item'
    ...
    function dirDropRow(e, rowId) {
        ...
        if (!data.type) return;
        var row = DIR_ROWS.find(function(r) {
            return r.id === rowId;
        });
        if (!row) return;
        var blank = row.steps.find(function(s) {
            return s.type === 'blank';
        });
        if (blank) {
            blank.type = data.type;
            blank.label = data.label;
            blank.cls = data.cls;
            blank.icon = data.icon;
        } else row.steps.push({
            type: data.type,
            label: data.label,
            cls: data.cls,
    ...
    function dirDropStep(e, rowId, si) {
        e.preventDefault();
        e.stopPropagation();
        document.querySelectorAll('.dstep').forEach(function(s) {
            s.classList.remove('dov');
        });
        var data = JSON.parse(e.dataTransfer.getData('text/plain'));
        if (!data.type) return;
        var row = DIR_ROWS.find(function(r) {
    ...
    function dirAutoPlace(type, label, cls, icon) {
        for (var i = 0; i < DIR_ROWS.length; i++) {
            var b = DIR_ROWS[i].steps.find(function(s) {
                return s.type === 'blank';
            });
            if (b) {
                b.type = type;
    ...
    function dirCloseModal() {
        document.getElementById('dirModal').classList.remove('on');
    }
    document.getElementById('dirModal').addEventListener('click', function(e) {
        if (e.target === this) dirCloseModal();
    });
